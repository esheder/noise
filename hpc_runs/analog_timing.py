...
from noise.analog import AnalogParameters, signal_make
...
if __name__ == '__main__':
    ...
    if argument in defaults:
        ...
        raise ValueError("Argument must be of a given set: "
                         f"{arguments}")
    par = AnalogParameters.from_dubi(multiplicity=multiplicity, **defaults)
    elapsed, (ts, curve, sigma) = time_f(feynman_y_by_signal)(par, t, rand_gen=generator, signal_generator=signal_make)
    mask = (ts <= 1e-1)
    tfit, cfit, sfit = ts[mask], curve[mask], sigma[mask]
    popt, pcov = fit_to_feynman_y(tfit, cfit, sfit)
    with (args.pdir / f'analog_timing.{argument}.{index}.{seed}').open('w') as f:
